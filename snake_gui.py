...
def draw_console(score, action_results, game_ended=False):
    ...
    for result in action_results[-10:]:
        ...
        if 'action' in display_result and isinstance(display_result['action'], int):
            display_result['action'] = ACTION_NAMES.get(display_result['action'], display_result['action'])
        result_text = font.render(str(display_result), True, BLACK)
